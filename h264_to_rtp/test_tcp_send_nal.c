...
#define RTP_PAYLOAD_MAX_SIZE    1400                          //need ?change 1400
#define MAX_NAL_BUF_SIZE        (1500 * 100)
#define SO_MAXCONN              10
#define DEFAULT_PORT            1893
#define SEND_BUF_SIZE           1500                          //change 1500
#define SSRC_NUM                10
...
int h264naltortp(int framerate, uint8_t *nal_buf, int nalu_len, int sock_fd);   //not use
...
int main(int argc, char **argv)
{
    ...
    char *myname = argv[0];
    char remote_host[32] = "192.168.1.2";
...
*/
    debug_print();
    h264file = fopen("stream_chn0.h264", "r");
    if (!h264file) {
        perror("fopen");
        exit(1);
    }
