...
void decode_rtp2h264(uint8_t *rtp_buf, int len, FILE *savefp)
{
    ...
    if (nalu_header->type == 28) { /* FU-A */
        ...
        if (fu_header->e == 1) { /* end of fu-a */
            fwrite(&rtp_buf[14], 1, len - 14, savefp);debug_print("end of fu-a");
            ...
            fwrite(&rtp_buf[14], 1, len - 14, savefp);debug_print("start of fu-a");
        } else { /* middle of fu-a */
            fwrite(&rtp_buf[14], 1, len - 14, savefp);debug_print("middle of fu-a");
        }
    } else { /* nalu */
        fputc(0, savefp);
        fputc(0, savefp);
        ...
        h264_nal_header = (nalu_header->type & 0x1f) 
            ...
            | (nalu_header->f << 7);
        fputc(h264_nal_header, savefp);
        fwrite(&rtp_buf[13], 1, len - 13, savefp);debug_print("nalu");
    }
...
{
    int ret;
    uint8_t recv_buf[2000];                              //need change?2000
    int sock_fd;
