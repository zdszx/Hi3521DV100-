...
void SAMPLE_VIO_Usage(char *sPrgNm)
{
    ...
    printf("\t0:  VI 1080P input, HD0(VGA),HD1(HDMI)/SD VO \n");
	printf("\t1:  SAMPLE_VIO_VENC_4K\n");
	printf("\tq:  quit\n");
...
/******************************************************************************
* function : VI(1080P: 8 windows) -> VPSS -> HD0(1080P,VGA)
                                         -> HD1(1080P,HDMI)
                                         -> SD0(D1)
******************************************************************************/
HI_S32 SAMPLE_VIO_4_1080P(HI_VOID)
{
	...
	HI_U32 u32ViChnCnt = VIU_MAX_DEV_NUM;
	HI_S32 s32VpssGrpCnt = VIU_MAX_DEV_NUM;
...
#if 0
	/******************************************
	 step 5: start vo HD0 (VGA), multi-screen, you can switch mode
	******************************************/
	printf("start vo HD0.\n");
	VoDev = SAMPLE_VO_DEV_DHD0;
	VoLayer = SAMPLE_VO_LAYER_VHD0;
	u32WndNum = u32ViMaxDevCnt;
	enVoMode = VO_MODE_4MUX;

	stVoPubAttr_hd0.enIntfSync = VO_OUTPUT_1080P60;
	stVoPubAttr_hd0.enIntfType = VO_INTF_VGA;
...
******************************************************************************/
HI_S32 SAMPLE_VIO_VENC_4K()
{

    SAMPLE_VI_MODE_E enViMode = SAMPLE_VI_MODE_1_4Kx2K;  //ifchange to720Pwil incur  AD failed
    ...
    HI_S32 ch;
    HI_U32 u32WndNum;
    HI_U32 u32ViMaxDevCnt;

      VENC_CHN VencChn;
    SAMPLE_RC_E enRcMode= SAMPLE_RC_AVBR; //change?

    PAYLOAD_TYPE_E enPayLoad[2]= {PT_H264, PT_H264};
    PIC_SIZE_E enSize[3] = {PIC_UHD4K, PIC_HD1080,PIC_HD720};          //Add 720
	HI_U32 u32Profile = 0;
    ...
    ******************************************/
    memset(&stVbConf,0,sizeof(VB_CONF_S));
    u32BlkSize = SAMPLE_COMM_SYS_CalcPicVbBlkSize(enNorm,\
                enSize[2], SAMPLE_PIXEL_FORMAT, SAMPLE_SYS_ALIGN_WIDTH,COMPRESS_MODE_SEG); //change PIC
    stVbConf.u32MaxPoolCnt = 128;
    ...
    ******************************************/
    ...
    /******************************************
     step 3: start vi dev & chn
    ******************************************/
    s32Ret = SAMPLE_COMM_VI_Start(enViMode, enNorm);          //affect
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("start vi failed!\n");
        goto END_HDZOOMIN_0;
    ...
    /******************************************
     step 4: start vpss and vi bind vpss
    ******************************************/
    s32Ret = SAMPLE_COMM_SYS_GetPicSize(enNorm, enSize[2], &stSize);      //change PIC
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("SAMPLE_COMM_SYS_GetPicSize failed!\n");
        goto END_HDZOOMIN_0;
    ...
    s32Ret = SAMPLE_COMM_VI_BindVpss(enViMode);                     //affect
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Vi bind Vpss failed!\n");
        goto END_HDZOOMIN_2;
    ...
    ******************************************/
	printf("start vo HD0.\n");
	VoDev = SAMPLE_VO_DEV_DHD0;                                //change??
	VoLayer = SAMPLE_VO_LAYER_VHD0;
	u32WndNum = 1;
	enVoMode = VO_MODE_1MUX;

	stVoPubAttr.enIntfSync = VO_OUTPUT_1080P25;             //change P25  SAMPLE_COMM_VO_GetWH
...
VpssGrp = 0;
        s32Ret = SAMPLE_COMM_VENC_Start(VencChn, enPayLoad[0],\
                                   VIDEO_ENCODING_MODE_PAL, enSize[2], enRcMode,u32Profile);  //change enNorm enSize
    if (HI_SUCCESS != s32Ret)
    {
    ...
    s32Ret = SAMPLE_COMM_VENC_BindVpss(VencChn, VpssGrp, VpssChn_VoHD0);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Start Venc failed!\n");
        goto END_HDZOOMIN_8;
...
int main(int argc, char *argv[])
{
    ...
    HI_MPI_SYS_GetChipId(&g_u32ChipId);

    signal(SIGINT, SAMPLE_VIO_HandleSig);
    signal(SIGTERM, SAMPLE_VIO_HandleSig);

    s32Ret = SAMPLE_VIO_VENC_4K();
