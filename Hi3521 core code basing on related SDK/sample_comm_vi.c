...
VI_DEV_ATTR_S DEV_ATTR_BT1120_INTERLEAVED_BASE =
{
    /*interface mode*/
...
VI_DEV_ATTR_S DEV_ATTR_4K_BASE =
{
    /*interface mode*/
    VI_MODE_BT1120_STANDARD,
    /*work mode, 1/2/4 multiplex*/
    ...
    /*timing info*/
    /*hsync_hfb    hsync_act    hsync_hhb*/                                          //change 3840 2160
    {0,            1280,        0,
    /*vsync0_vhb vsync0_act vsync0_hhb*/
     0,            720,        0,
    /*vsync1_vhb vsync1_act vsync1_hhb*/
...
HI_S32 SAMPLE_COMM_VI_GetMaxDev(HI_U32 u32ChipId)
{
    if(HI3521D_V100 != u32ChipId)
         return VIU_MAX_DEV_NUM / 2;
    return VIU_MAX_DEV_NUM;
}
...
HI_S32 SAMPLE_COMM_VI_Mode2Size(SAMPLE_VI_MODE_E enViMode, VIDEO_NORM_E enNorm, RECT_S *pstCapRect, SIZE_S *pstDestSize)
{
    ...
    switch (enViMode)
    {
        ...
        case SAMPLE_VI_MODE_4_1080P:
            ...
            pstDestSize->u32Height = HD_HEIGHT;            //1920 1080
            pstCapRect->u32Width  = HD_WIDTH;
            pstCapRect->u32Height = HD_HEIGHT;
            break;
        case SAMPLE_VI_MODE_1_4Kx2K:
            pstDestSize->u32Width  = _720P_WIDTH;         //change 4K 3840 2160
            pstDestSize->u32Height = _720P_HEIGHT;
            pstCapRect->u32Width  = _720P_WIDTH;
            pstCapRect->u32Height = _720P_HEIGHT;
            break;
...
HI_S32 SAMPLE_COMM_VI_StartDev(VI_DEV ViDev, SAMPLE_VI_MODE_E enViMode)
{
    ...
    {
        case SAMPLE_VI_MODE_1_4Kx2K:
            memcpy(&stViDevAttr,&DEV_ATTR_4K_BASE,sizeof(stViDevAttr));                          //!
        ...
        case SAMPLE_VI_MODE_4_1080P:
        case SAMPLE_VI_MODE_2_1080P:
            memcpy(&stViDevAttr,&DEV_ATTR_BT1120_INTERLEAVED_BASE,sizeof(stViDevAttr));
            SAMPLE_COMM_VI_SetMask(ViDev,&stViDevAttr);
            break;
        case SAMPLE_VI_MODE_16_720P:
    ...
    if (s32Ret != HI_SUCCESS)
    {
        SAMPLE_PRT("ViDev%d HI_MPI_VI_SetDevAttr failed with %#x!\n", ViDev,s32Ret);
        return HI_FAILURE;
    }
...
HI_S32 SAMPLE_COMM_VI_Start(SAMPLE_VI_MODE_E enViMode, VIDEO_NORM_E enNorm)
{
    ...
    for(i=0; i<stViParam.s32ViChnCnt ; i++)
    {
        ...
        if (SAMPLE_VI_MODE_4_1080P == enViMode
         ...
         || SAMPLE_VI_MODE_2_1080P == enViMode)
        {
            /* When in the 4x1080p mode,0,4,8,16 */
           // if (ViChn%8 == 0)
            {
